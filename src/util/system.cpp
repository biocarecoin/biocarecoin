...
const char * const BITCOIN_CONF_FILENAME = "biocarecoin.conf";
...
#else
    const char* pszModule = "biocarecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Biocarecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Biocarecoin";
#else
    // Unix
    return pathRet / ".biocarecoin";
#endif
#endif
}
